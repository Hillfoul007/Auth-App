...
const connectDB = async () => {
  try {
    const username = process.env.MONGODB_USERNAME || "sunflower110001";
    const password = process.env.MONGODB_PASSWORD || "your_password_here";
    const mongoURI = `mongodb+srv://${username}:${password}@cluster0.ic8p792.mongodb.net/${process.env.MONGODB_DATABASE || "homeservices"}?retryWrites=true&w=majority`;
...
authRouter.post("/verify-token", async (req, res) => {
  try {
    ...
    // For demo purposes, accept any JWT or Firebase token
    if (token.startsWith("jwt_") || token.startsWith("firebase_")) {
      const mockUser = {
